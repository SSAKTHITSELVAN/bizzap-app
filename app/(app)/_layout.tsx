...
// import { Dimensions, StyleSheet, Text, TouchableOpacity, View, DeviceEventEmitter, BackHandler } from 'react-native';
...
//         // Use pathname for more reliable active state tracking
//         if (pathname?.includes('/dashboard')) return 0;
//         if (pathname?.includes('/search')) return 1;
//         if (pathname?.includes('/bizzapai')) return 2;
...
//             // Check if we're on a main tab
//             const isOnMainTab = mainRoutes.some(route => pathname?.includes(route.split('/').pop() || ''));

//             if (isOnMainTab) {
//                 // If on main tab, go to dashboard or exit
//                 if (pathname?.includes('/dashboard')) {
//                     // On dashboard, exit app
//                     BackHandler.exitApp();
//                     return true;
//                 } else {
//                     // On other tabs, go to dashboard
//                     router.push('/(app)/dashboard');
//                     return true;
//                 }
//             }

//             // For nested routes, use default back behavior
//             return false;
//         };
...
function CustomTabBar({ state, descriptors, navigation, unreadCount, onTabPress }: CustomTabBarProps) {
    ...
    // UPDATED getActiveIndex: Return -1 if not on a main tab
    const getActiveIndex = () => {
        if (pathname?.includes('/dashboard')) return 0;
        if (pathname?.includes('/search')) return 1;
        if (pathname?.includes('/bizzapai')) return 2;
        if (pathname?.includes('/notification')) return 3;
        if (pathname?.includes('/chat')) return 4;
        return -1; // Default to none (allows navigation from Profile back to Dashboard)
    };

    const activeIndex = getActiveIndex();

    // UPDATED handleNavigation: Check index to prevent re-navigation
    const handleNavigation = (route: string, index: number) => {
        if (activeIndex === index) return; // Prevent if already on this tab
        onTabPress(route);
        router.push(route as any);
    };
    ...
    return (
        <View style={styles.tabBarWrapper}>
            <View style={[
                ...
                <View style={styles.tabBarContent}>
                    <TabButton icon={(isActive) => <LeadIcon color={isActive ? '#FFFFFF' : '#8FA8CC'} isActive={isActive} />} label="Lead" isFocused={activeIndex === 0} onPress={() => handleNavigation('/(app)/dashboard', 0)} />
                    <TabButton icon={(isActive) => <SearchIcon color={isActive ? '#FFFFFF' : '#8FA8CC'} isActive={isActive} />} label="Search" isFocused={activeIndex === 1} onPress={() => handleNavigation('/(app)/search', 1)} />
                    <TabButton icon={(isActive) => <PlusIcon isActive={isActive} />} label="" isFocused={activeIndex === 2} onPress={() => handleNavigation('/(app)/bizzapai', 2)} isCenter />
                    <TabButton icon={(isActive) => <NotificationIcon color={isActive ? '#FFFFFF' : '#8FA8CC'} isActive={isActive} />} label="Alerts" isFocused={activeIndex === 3} onPress={() => handleNavigation('/(app)/notification', 3)} />
                    <TabButton icon={(isActive) => <ChatIcon color={isActive ? '#FFFFFF' : '#8FA8CC'} hasUnread={unreadCount > 0} isActive={isActive} />} label="Chat" isFocused={activeIndex === 4} onPress={() => handleNavigation('/(app)/chat', 4)} />
                </View>
            </View>
        </View>
    );
